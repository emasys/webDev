<?php $title = 'Welcome open online diaries'; ?>
...
t.user_id = u.user_id  GROUP BY (p.thread_id)
...
<thead>
 ...
 <th><strong>Latest Comments</strong>:</th>
 </tr>
</thead>';
...
MYSQLI_ASSOC)) {
 ...
 echo '<tr>
 <td align="left"><a
href="read.php?tid=' . $row['thread_id'] .
'">' . $row['subject'] . '</a></td>
 
 <td align="center">' . $row['first'] .
'</td>
 <td align="center">' .
$row['responses'] . '</td>
<td align="center">' . $row['last'] .
'</td>
 </tr>';
...
'<a href="explore.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
 }

 // Make all the numbered pages:
 for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="explore.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
 ...
 echo '<a href="explore.php?s=' .($start + $display) . '&p=' . $pages .'">Next</a>';
 }
