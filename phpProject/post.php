...
is_numeric($_POST['tid']) ) {
 ...
 $body = FALSE;
 echo '<p>Please enter a body for
this post.</p>';
 }
 ...
 $q = "INSERT INTO threads (user_id, subject) VALUES ({$_SESSION['user_id']},
  '" . mysqli_real_escape_string($dbc, $subject) . "')";
 $r = mysqli_query($dbc, $q);
 if (mysqli_affected_rows($dbc) == 1) { $tid = mysqli_insert_id($dbc);
 } else {
...
$q = "INSERT INTO posts (thread_id,
user_id, message, posted_on) VALUES
($tid, {$_SESSION['user_id']}, '" .
mysqli_real_escape_string($dbc, $body)
. "', UTC_TIMESTAMP())";
 $r = mysqli_query($dbc, $q);
 if (mysqli_affected_rows($dbc) == 1) {
 header("refresh:0; url=read.php?tid=$tid")
 /*echo '<p>Your post has been
entered.</p>'*/;
 } else {
	 
 echo '<p>Your post could not be
handled due to a system
error.</p>';
 }
